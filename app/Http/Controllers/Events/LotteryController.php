...
class LotteryController extends Controller
{
    ...
    // Perform a draw for the active lottery and settle payouts.
    // NOTE: This endpoint triggers settlement for all entries of the active event.
    public function draw(Request $request)
    {
        $event = Event::where('is_active', true)->where('key', 'lottery')->first();
        // Protect draw endpoint: only allow in local environment or when a valid secret header is provided.
        $secret = env('EVENT_DRAW_SECRET');
        $provided = $request->header('X-EVENT-DRAW-SECRET');
        if (!app()->environment('local') && (empty($secret) || $provided !== $secret)) {
        ...
        // Compute jackpot as sum of stakes
        $entries = EventLottery::where('event_id', $event->id)->get();
        $jackpot = $entries->sum('stake');
        ...
        foreach ($entries as $entry) {
            $matches = count(array_intersect($entry->numbers, $drawNumbers));
            if ($matches >= 3 && $matches <= 6) {
                $winners[$matches][] = $entry;
            }
        }

        // Original jackpot before payouts
        $originalJackpot = $jackpot;

        DB::beginTransaction();
        try {
            $totalPaid = 0;
            // Pay tiers other than 6 first
            foreach ([5,4,3] as $tier) {
                $count = count($winners[$tier]);
                if ($count === 0) continue;
                $poolAmount = (int) floor($tierPercents[$tier] * $originalJackpot);
                // per winner
                $perWinner = (int) floor($poolAmount / $count);
                if ($tier === 3 && $perWinner < 1) $perWinner = 1;
                foreach ($winners[$tier] as $entry) {
                    // credit user
                    $user = $entry->user;
                    $user->balance = ($user->balance ?? 0) + $perWinner;
                    $user->save();

                    $entry->payout = $perWinner;
                    $entry->save();
                    $totalPaid += $perWinner;
                }
            }

            // Handle 6-match: full jackpot split among winners (if any)
            $sixCount = count($winners[6]);
            if ($sixCount > 0) {
                $perWinner = (int) floor($originalJackpot / $sixCount);
                foreach ($winners[6] as $entry) {
                    $user = $entry->user;
                    $user->balance = ($user->balance ?? 0) + $perWinner;
                    $user->save();
                ...
                // jackpot reset to 0 when won
                $event->meta = array_merge($event->meta ?? [], ['draw_numbers' => $drawNumbers]);
                $event->save();
            } else {
                // No jackpot winner; record draw numbers and leave remaining jackpot intact
                $event->meta = array_merge($event->meta ?? [], ['draw_numbers' => $drawNumbers]);
                $event->save();
            }
            ...
            // Prepare response for current user
            $currentUser = $request->user();
            $myEntry = null;
            if ($currentUser) {
                $myEntry = EventLottery::where('event_id', $event->id)->where('user_id', $currentUser->id)->latest()->first();
            }
            ...
            return response()->json([
                ...
                'draw' => $drawNumbers,
                'my_entry' => $myEntry,
                'jackpot' => $originalJackpot,
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Draw error: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Draw failed'], 500);
        }
    }
